...
prob_fish <- problem(cost_fish, features = features_stack) %>%
  ...
  add_gurobi_solver(gap = 0.1, time_limit = 2700)
...
tm_raster(palette = c("#c6c5c5", "#409a00"), n=2, legend.show = FALSE) +
  tm_layout(panel.labels = "Artisanal Fishing", panel.label.bg.color = "steelblue4", panel.label.color = 'white', panel.label.size = 1)
...
# Boundary Penalty = 0.1
```{r}
### Now lets try setting a boundary length of 0.1
### OBJECTIVE: Minimize fishing pressure while meeting 20% representation targets, locking in MPAs, locking out oil rigs, and a boundary penalty of 0.1
...
## Solve problem
sprob_fish_blm <- solve(prob_fish_blm)

## Plot the solution to see what it looks like
plot(sprob_fish_blm,  main = c("BLM = 0.1"))
```
