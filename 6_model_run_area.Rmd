...
prob_area_blm_ <- problem(cost_area, features = features_stack) %>%
  add_min_set_objective() %>%
  add_relative_targets(0.2) %>%
  add_locked_in_constraints(mpa_raster) %>%
  add_locked_out_constraints(oil_raster) %>%
...
prob_area_blm_10 <- problem(cost_area, features = features_stack) %>%
  add_min_set_objective() %>%
  add_relative_targets(0.2) %>%
  add_locked_in_constraints(mpa_raster) %>%
  add_locked_out_constraints(oil_raster) %>%
...
## Print the problem to verify it is as expected   
print(prob_area_blm_10)

## Solve problem
sprob_area_blm_10 <- solve(prob_area_blm_10)

## Plot the solution to see what it looks like
plot(sprob_area_blm_10,  main = c("BLM = 10"))
...
plotstack_area_blm <- stack(sprob_area_blm, sprob_area_blm_1, sprob_area_blm_10)
